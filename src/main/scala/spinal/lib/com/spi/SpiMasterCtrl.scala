  ...
  val io = new Bundle {
    ...
    def driveFrom(bus : BusSlaveFactory, baseAddress : Int = 0)(generics : SpiMasterCtrlMemoryMappedConfig) = new Area {
      ...
      val interruptCtrl = new Area {
        ...
        val cmdFifoHalfIntEnable = bus.createReadAndWrite(Bool(), address = baseAddress + 4, 2) init(False)
        val rspFifoHalfIntEnable  = bus.createReadAndWrite(Bool(), address = baseAddress + 4, 3) init(False)
        ...
        val cmdFifoHalfFreeInt = bus.read(cmdFifoHalfIntEnable & cmdLogic.fifoAvailability > cmdFifoDepth / 2, address = baseAddress + 4, 10)
        val rspFifoHalfFullInt = bus.read(rspFifoHalfIntEnable & rspLogic.fifoOccupancy > rspFifoDepth / 2, address = baseAddress + 4, 11)
        val interrupt = rspInt || cmdInt || cmdFifoHalfFreeInt || rspFifoHalfFullInt
      }
