...
// Adopted to SDMMC interface by Ruslan Zalata <rz@fabmicro.ru>
...
static uint8_t g_buf[512+2]; // RZ: SDMMC I/O implies reading CRC16 right after data block 
...
//------------------------------------------------------------------------------
Fat* find_fat_volume(const char* name, int len)
{
...
static int sync_buf(Fat* fat)
{
  ...
  {
    if (!fat->ops.write(fat->dev, fat->buf, fat->sect))
      return FAT_ERR_IO;
...
static int update_buf(Fat* fat, uint32_t sect)
{
  if (fat->sect != sect)
  {
    ...
    if (!fat->ops.read(fat->dev, fat->buf, sect))
      return FAT_ERR_IO;
...
static int follow_path_RZ(Dir* dir, const char** path, Loc* loc)
{
  int err, len;
  uint32_t dir_clust;
  bool dir_enterable;
...
// RZ: Allow empty-named volumes
//  if (len == 0)
//    return FAT_ERR_PATH;
  if(len == 0 || len == strlen(str))
    dir->fat = find_fat_volume(NULL, 0);
  else
    dir->fat = find_fat_volume(str, len);
...
static bool get_part_lba(uint8_t* buf, int partition, uint32_t* lba)
{
  ...
  if (mbr->part[partition].type != 0x0c) // Must be FAT32 with LBA
    return false;
...
//------------------------------------------------------------------------------
int probe(DiskOps* ops, int dev, int partition, uint32_t* lba)
{
  *lba = 0;
  if (!ops->read(dev, g_buf, *lba))
    return FAT_ERR_IO;
...
int fat_probe(DiskOps* ops, int dev, int partition)
{
  uint32_t lba;
  return probe(ops, dev, partition, &lba);
}
...
int fat_mount(DiskOps* ops, int dev, int partition, Fat* fat, const char* name)
{
  uint32_t lba;
  int err = probe(ops, dev, partition, &lba);
  if (err)
    return err;
  ...
  // Load FsInfo
  if (!ops->read(fat->dev, g_buf, fat->info_sect))
    return FAT_ERR_IO;
...
int fat_file_read(File* file, void* buf, int len, int* bytes)
{
  *bytes = 0;
  ...
  //file->flags |= FAT_ACCESSED; // RZ: Prevent writing dir entry to media on reads
...
int fat_file_seek(File* file, int offset, int seek)
{
  ...
  if (file->sect != ssect)
  {
    ...
    {
      if (!file->fat->ops.write(file->fat->dev, file->buf, ssect))
        return FAT_ERR_IO;
      file->flags &= ~FAT_FILE_DIRTY;
    }

    if (!file->fat->ops.read(file->fat->dev, file->buf, file->sect))
      return FAT_ERR_IO;
  }
...
int fat_file_sync(File* file)
{
  ...
  if (file->flags & FAT_MODIFIED) // RZ: Do not write back unmodified data block
    if (!file->fat->ops.write(file->fat->dev, file->buf, file->sect))
      return FAT_ERR_IO;
